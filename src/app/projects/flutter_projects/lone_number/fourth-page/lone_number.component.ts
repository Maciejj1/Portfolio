import { Component } from '@angular/core';

@Component({
  selector: 'app-lone-number',
  templateUrl: './lone_number.component.html',
  styleUrls: ['../../../../../styles.scss'],
})
export class LoneNumberPageComponent {
  mysaving = 'assets/mobile_apps/lonenumber.png';
  lonelogo = 'assets/mobile_apps/lone-logo.png';
  background = 'assets/shapes.png';
  googleplay = 'assets/mobile_apps/Google-Play.png'
  appstore = 'assets/mobile_apps/App-Store.png'
  github = 'assets/github.png';

  //contstructed

  flutter = 'assets/mobile/flutter.png';
  firebase = 'assets/frontend/firebase.png';
  bloc = 'assets/mobile/bloc.png';
  dart = 'assets/mobile/dart.png';
  stripe = 'assets/mobile/stripe.png';
}
